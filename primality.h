...
void mpz_pX_mod_mult(mpz_pX& rop, const mpz_pX x, const mpz_pX y, mpz_class mod, unsigned int polymod);
void mpz_pX_mod_power(mpz_pX &rop, const mpz_pX &x, mpz_class power, mpz_class mult_mod, unsigned int polymod);
...
void mpz_pX::compact() {
	...
	if(degree != i) {
		coef = (mpz_class **)realloc(coef,sizeof(mpz_class*)*(degree+1));
	degree = i;
	}
}


void mpz_pX_mod_mult(mpz_pX& rop, const mpz_pX _x,const mpz_pX _y, mpz_class mod,unsigned int polymod) {
	mpz_pX x = _x;
	mpz_pX y = _y;
	rop.clear();
	...
	unsigned int i;
	for(i=0; i<=k; i++) {
	sum += x.getCoef(i)*(y.getCoef(k-i)+y.getCoef(k+polymod-i));
	}	
...
void mpz_pX_mod_power(mpz_pX &rop, const mpz_pX& x,mpz_class power, mpz_class mult_mod,unsigned int poly_mod) {
	rop.clear();
	rop.setCoef(1,0);
